    ...
    if ((length input_file_path_name) > 0 && (length output_file_path_name) > 5 && (length output_gif_file_name) > 4)
      then do
        forkIO $ do
          ...
          if result == 1
            then do
              entrySetText status_entry "Did not work. Check your settings."
            else do entrySetText status_entry "Ready."
          forkIO $ do
            open_gif output_file_path_name
            return ()
          return ()
        return ()
      else do entrySetText status_entry "File paths are wrong. Check your settings."
    ...
    if fileExists
      then do
        forkIO $ do
          ...
          return ()
        return ()
      else do entrySetText status_entry "GIF does not exist. Check your settings."
